...
function resetTheGame() {
    score = 0;
    for(let row = 0; row < gameField.length; row++)for(let cell = 0; cell < gameField.length; cell++)gameField[row][cell] = 0;
}
function changingDivMessage(value) {
    for (const child of gameStatus.children)if (child.classList.contains(`message-${value}`)) child.classList.remove("hidden");
...
function renderGameField() {
    ...
    for(let x = 0; x < gameField.length; x++){
        for(let y = 0; y < gameField.length; y++)if (gameField[x][y] !== 0) {
            ...
            else rows[x].children[y].classList.add(`field-cell--${gameField[x][y]}`);
        } else {
            rows[x].children[y].className = "field-cell";
...
function checkRowForGameOver(field) {
    ...
    field.map((r)=>{
        return r.filter((digit)=>{
            for(let i = 0; i < gameField.length; i++)if (r[i] === r[i + 1] || r[i] === 0) {
                rowStatus = true;
                return;
            }
        });
...
function slidingSum(row, key) {
    ...
    for(let cell = 0; cell < fieldRow.length; cell++)if (fieldRow[cell] === fieldRow[cell + 1]) {
        fieldRow[cell] *= 2;
        fieldRow[cell + 1] = 0;
        score += fieldRow[cell];
    }
...
function moveUpAndDown(key) {
    for(let r = 0; r < gameField.length; r++){
        ...
        for(let c = 0; c < gameField.length; c++)gameField[c][r] = transformRow[c];
    }
}
